...
public:
    ...
    // Constructor for Monocular cameras with gps.
    Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, const cv::Point3d tGPS, Frame* pPrevF = static_cast<Frame*>(NULL));
