...
public:
    ...
    cv::Mat DrawFrame2(float imageScale=1.f);
    cv::Mat DrawRightFrame(float imageScale=1.f);

    bool both;

    // 投影到当前帧的3D点
    vector<Eigen::Vector3d> mvCurrent3DP;

    // 投影到当前帧的像素坐标
    vector<Eigen::Vector2d> mvCurrent2DP;

    // Delaunay三角剖分
    cv::Subdiv2D mSubdiv;
    std::mutex mSubdivMutex;

    //sunjinliang take from protected
    Frame mCurrentFrame;
