...
public:
    ...
    // Input images: RGB (CV_8UC3) or grayscale (CV_8U). RGB is converted to grayscale.
    // Returns the camera pose (empty if tracking fails).
    Sophus::SE3f TrackMonocular(const cv::Mat &im, const double &timestamp, const cv::Point3d tGPS = cv::Point3d(), string imagename ="", const vector<IMU::Point>& vImuMeas = vector<IMU::Point>(), string filename="");
