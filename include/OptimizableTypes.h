...
// GPS
class  EdgeSE3OnlyPoseWithGPS: public  g2o::BaseUnaryEdge<3, Eigen::Vector3d, g2o::VertexSE3Expmap>{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    EdgeSE3OnlyPoseWithGPS(){}
    ...
    void computeError()  {
        ...
        _error = obs - v1->estimate().translation().cast<double>();
    }
...
// GPS 
class  EdgeSE3ProjectXYZWithGPS: public  g2o::BaseBinaryEdge<3, Eigen::Vector3d, g2o::VertexSBAPointXYZ, g2o::VertexSE3Expmap>{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    EdgeSE3ProjectXYZWithGPS();
    ...
    void computeError()  {
        ...
        if(v2->estimate() == Eigen::Vector3d(0, 0, 0))
        {
            // GPS残差
            _error = obs - v1->estimate().translation().cast<double>();
        }
        ...
        {
            // 重投影残差
            Eigen::Vector2d pj = pCamera->project(v1->estimate().map(v2->estimate()));
            _error = obs - Eigen::Vector3d(pj(0), pj(1), 1);
        }
    }
