...
namespace ORB_SLAM3
{
    ...
    void Optimizer::GlobalBundleAdjustemntWithGPS(Map *pMap, Eigen::Matrix3Xd co_inecef, int nIterations, bool *pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
    {
        ...
        BundleAdjustmentWithGPS(vpKFs, vpMP, co_inecef, nIterations, pbStopFlag, nLoopKF, bRobust);
    }

    void Optimizer::BundleAdjustmentWithGPS(const vector<KeyFrame *> &vpKFs, const vector<MapPoint *> &vpMP,
                                            Eigen::Matrix3Xd co_inecef, int nIterations, bool *pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
    {
        ...
        vector<ORB_SLAM3::EdgeSE3ProjectXYZWithGPS *> vpEdgesMono;
        ...
        for (size_t i = 0; i < vpMP.size(); i++)
        {
            ...
            for (map<KeyFrame *, tuple<int, int>>::const_iterator mit = observations.begin(); mit != observations.end(); mit++)
            {
                ...
                if (1) // 单目相机
                {
                    ...
                    obs << kpUn.pt.x, kpUn.pt.y, 1;

                    ORB_SLAM3::EdgeSE3ProjectXYZWithGPS *e = new ORB_SLAM3::EdgeSE3ProjectXYZWithGPS();
...
// GPS残差
#if 0
        g2o::VertexSBAPointXYZ *vPoint = new g2o::VertexSBAPointXYZ();
        vPoint->setEstimate(Eigen::Vector3d(0.f, 0.f, 0.f));    // 相机空间坐标
        preMaxID += 1;
        vPoint->setId(preMaxID);
        vPoint->setMarginalized(true);
        optimizer.addVertex(vPoint);
#endif
        ...
        for (size_t i = 0; i < vpKFs.size(); i++)
        {
            ...
            // pKF->mnId
            Eigen::Matrix<double, 3, 1> obs;
            obs << pKF->mInECEF.x, pKF->mInECEF.y, pKF->mInECEF.z;

            ORB_SLAM3::EdgeSE3OnlyPoseWithGPS *e = new ORB_SLAM3::EdgeSE3OnlyPoseWithGPS();

            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(pKF->mnId)));
            e->setMeasurement(obs);

            // GPS逆的协方差矩阵
            e->setInformation(co_inecef);
        ...
        for (size_t i = 0; i < vpKFs.size(); i++)
        {
            ...
            else
            {
                ...
                if (dist > 1)
                {
                    ...
                    {
                        ORB_SLAM3::EdgeSE3ProjectXYZWithGPS *e = vpEdgesMono[i2];
    ...
    int Optimizer::PoseOptimizationWithGPS(Frame *pFrame, GeoTransform geo)
    {
        g2o::SparseOptimizer optimizer;
        g2o::BlockSolver_6_3::LinearSolverType *linearSolver;
        ...
        if ( geo.s > 0.01 )
        {
            Eigen::Vector3d ecef(pFrame->mECEF.x, pFrame->mECEF.y, pFrame->mECEF.z);
            Eigen::Vector3d inecef = (geo.R.transpose()*(ecef-geo.T))/geo.s;

            Eigen::Matrix<double, 3, 1> obs;
            obs<< inecef(0), inecef(1), inecef(2);

            ORB_SLAM3::EdgeSE3OnlyPoseWithGPS *e = new ORB_SLAM3::EdgeSE3OnlyPoseWithGPS();
            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(0)));
            e->setMeasurement(obs);
            e->setInformation(Eigen::Matrix3d::Identity()); // 只有一条边，设为单位矩阵
    ...
    void Optimizer::LocalBundleAdjustmentWithGPS(KeyFrame *pKF, Eigen::Matrix3Xd co_inecef, bool *pbStopFlag, Map *pMap, int &num_fixedKF, int &num_OptKF, int &num_MPs, int &num_edges)
    {
        // Local KeyFrames: First Breath Search from Current Keyframe
        list<KeyFrame *> lLocalKeyFrames;
        ...
        for (list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end(); lit != lend; lit++)
        {
            ...
            Eigen::Matrix<double, 3, 1> obs;
            obs << pKFi->mInECEF.x, pKFi->mInECEF.y, pKFi->mInECEF.z;
