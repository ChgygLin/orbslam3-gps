...
namespace ORB_SLAM3 {
    ...
    // GPS 
    bool EdgeSE3OnlyPoseWithGPS::read(std::istream& is){
        for (int i=0; i<3; i++){
            is >> _measurement[i];
        }
        for (int i=0; i<3; i++)
    ...
    bool EdgeSE3OnlyPoseWithGPS::write(std::ostream& os) const {
    ...
    void EdgeSE3OnlyPoseWithGPS::linearizeOplus() {
        g2o::VertexSE3Expmap * vi = static_cast<g2o::VertexSE3Expmap *>(_vertices[0]);

        // GPS残差雅可比
        Eigen::Vector3d tc = vi->estimate().translation().cast<double>();

        _jacobianOplusXi << 0.f, tc(2), -tc(1), 1.f, 0.f, 0.f,
                            -tc(2), 0.f, tc(0), 0.f, 1.f, 0.f,
                            tc(1), -tc(0), 0.f, 0.f, 0.f, 1.f;
    }
    ...
    // GPS
    EdgeSE3ProjectXYZWithGPS::EdgeSE3ProjectXYZWithGPS() : BaseBinaryEdge<3, Eigen::Vector3d, g2o::VertexSBAPointXYZ, g2o::VertexSE3Expmap>() {
    }

    bool EdgeSE3ProjectXYZWithGPS::read(std::istream& is){
        for (int i=0; i<3; i++){
            is >> _measurement[i];
        }
        for (int i=0; i<3; i++)
    ...
    bool EdgeSE3ProjectXYZWithGPS::write(std::ostream& os) const {
    ...
    void EdgeSE3ProjectXYZWithGPS::linearizeOplus() {
        g2o::VertexSE3Expmap * vj = static_cast<g2o::VertexSE3Expmap *>(_vertices[1]);
        g2o::SE3Quat T(vj->estimate());
        g2o::VertexSBAPointXYZ* vi = static_cast<g2o::VertexSBAPointXYZ*>(_vertices[0]);
        ...
        auto projectJac = -pCamera->projectJac(xyz_trans);
        Eigen::Matrix<double, 3, 3> projectJac_q;
        projectJac_q << projectJac(0, 0), projectJac(0, 1), projectJac(0, 2),
                        projectJac(1, 0), projectJac(1, 1), projectJac(1, 2),
                        0.f, 0.f, 0.f;  

        _jacobianOplusXi =  projectJac_q * T.rotation().toRotationMatrix();

        if (xyz == Eigen::Vector3d(0, 0, 0))
        {
            // GPS残差雅可比
            Eigen::Vector3d tc = vj->estimate().translation().cast<double>();

            _jacobianOplusXj << 0.f, tc(2), -tc(1), 1.f, 0.f, 0.f,
                                -tc(2), 0.f, tc(0), 0.f, 1.f, 0.f,
                                tc(1), -tc(0), 0.f, 0.f, 0.f, 1.f;
        }
        else
        {
            ...
            _jacobianOplusXj = projectJac_q * SE3deriv;
        }
    }
    // GPS END
