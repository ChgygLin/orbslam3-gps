    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    else if(mSensor==GPS_MONOCULAR)
        cout << "Monocular-GPS" << endl;
    ...
    if(mStrLoadAtlasFromFile.empty())
    {
        ...
        cout << "Vocabulary loaded in " << (double)(clock() - tStart)/CLOCKS_PER_SEC << "s" << endl << endl;
    ...
    mpLocalMapper = new LocalMapping(this, mpAtlas, mSensor==MONOCULAR || mSensor==IMU_MONOCULAR || mSensor==GPS_MONOCULAR,
    ...
    mpLoopCloser = new LoopClosing(mpAtlas, mpKeyFrameDatabase, mpVocabulary, mSensor!=MONOCULAR && mSensor!=GPS_MONOCULAR, activeLC); // mSensor!=MONOCULAR);
...
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const cv::Point3d tGPS, string imagename, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    if(mSensor!=MONOCULAR && mSensor!=IMU_MONOCULAR && mSensor!=GPS_MONOCULAR)
    {
    ...
    Sophus::SE3f Tcw;
    if(mSensor == System::GPS_MONOCULAR)
        Tcw = mpTracker->GrabImageMonocular(imToFeed, timestamp, imagename, tGPS);
    else
        Tcw = mpTracker->GrabImageMonocular(imToFeed,timestamp,imagename);
...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    if(mSensor==MONOCULAR || mSensor==GPS_MONOCULAR)
    {
