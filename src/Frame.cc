...
Frame::Frame(const Frame &frame)
    :mpcpi(frame.mpcpi),mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
     ...
     mTcw(frame.mTcw), mbHasPose(false), mbHasVelocity(false), mGPS(frame.mGPS), mECEF(frame.mECEF)
{
...
// Monocular with GPS
Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, const cv::Point3d tGPS, Frame* pPrevF)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     mTimeStamp(timeStamp), mK(static_cast<Pinhole*>(pCamera)->toK()), mK_(static_cast<Pinhole*>(pCamera)->toK_()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),mGPS(tGPS),
     ...
     mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    // mGPS = cv::Point3d(tGPS.x, tGPS.y, tGPS.z);
    mECEF = LLA2ECEF(mGPS);
    //cout<<"Frame gps: "<<mGPS<<endl;
    //cout<<"Frame ecef: "<<mECEF<<endl;
