...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    if(mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR || mSensor==System::GPS_MONOCULAR)
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename, const cv::Point3d tGPS)
{
    mImGray = im;
    if(mImGray.channels()==3)
    {
    ...
    else if(mSensor == System::GPS_MONOCULAR)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames)
            mCurrentFrame = Frame(mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth, tGPS);
        else
            mCurrentFrame = Frame(mImGray,timestamp,mpORBextractorLeft,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth, tGPS);
    }
...
void Tracking::UpdateLastFrame()
{
    ...
    if(mnLastKeyFrameId==mLastFrame.mnId || mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR || mSensor==System::GPS_MONOCULAR || !mbOnlyTracking)
        return;
...
bool Tracking::TrackWithMotionModel()
{
    ...
    int nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,th,mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR || mSensor==System::GPS_MONOCULAR);
    ...
    if(nmatches<20)
    {
        ...
        nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,2*th,mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR || mSensor==System::GPS_MONOCULAR);
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    if(mSensor==System::MONOCULAR || mSensor==System::GPS_MONOCULAR)
        thRefRatio = 0.9f;
    ...
    const bool c1c = mSensor!=System::MONOCULAR && mSensor!=System::GPS_MONOCULAR && mSensor!=System::IMU_MONOCULAR && mSensor!=System::IMU_STEREO && mSensor!=System::IMU_RGBD && (mnMatchesInliers<nRefMatches*0.25 || bNeedToInsertClose) ;
...
void Tracking::CreateNewKeyFrame()
{
    ...
    if(mSensor!=System::MONOCULAR && mSensor!=System::IMU_MONOCULAR && mSensor!=System::GPS_MONOCULAR) // TODO check if incluide imu_stereo
    {
